...
5. **MF-06 WebKit serves white default theme on every route (P0).** Tailwind 4 compiles 96 design tokens under `@layer theme { :root, :host { ... } }` (verified in `.next/static/chunks/*.css`), a documented Safari compatibility hotspot (TW GitHub Discussions #15556 + #15284, official Tailwind 4 support floor Safari 16.4+). `var()` references fail to resolve in WebKit; body falls back to default white + Times-New-Roman + blue underlined links. Rules using hardcoded hex (like `.bg-gradient-flow`) still render, which explains the visible teal hero badge on an otherwise-broken page. Earlier hypotheses (`@property` on `<color>` types, `oklch()`) ruled out by direct compiled-CSS inspection on 2026-05-19. Affects 18 percent of EU desktop and 30 percent of EU mobile audience. Fix: strip `:host` from the generated theme rule via PostCSS, or use `@theme inline` (Tailwind v4.1+), plus Playwright CI gate asserting body computed background equals `rgb(10, 13, 20)`.
