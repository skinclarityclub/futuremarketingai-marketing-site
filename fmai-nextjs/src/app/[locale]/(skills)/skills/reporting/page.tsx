...
export async function generateMetadata({
  ...
  return generatePageMetadata({ locale, namespace: 'skills-reporting', path: '/skills/reporting' })
}

const FEATURE_KEYS = ['dashboards', 'weekly', 'trends', 'roi'] as const
const STEP_KEYS = ['integrate', 'analyze', 'report'] as const
const USE_CASE_KEYS = ['agencies', 'managers'] as const

export default async function ReportingPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations({ locale, namespace: 'skills-reporting' })
  ...
  return (
    <PageShell>
      ...
      <BreadcrumbJsonLd
        items={[
          { name: 'Home', path: '/' },
          { name: 'Reporting', path: '/skills/reporting' },
        ]}
        locale={locale}
      />
