...
const FEATURE_KEYS = ['static', 'video', 'copy', 'variants'] as const
const STEP_KEYS = ['brief', 'generate', 'launch'] as const
const USE_CASE_KEYS = ['agencies', 'performance'] as const

export default async function AdCreatorPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params
  setRequestLocale(locale)
  const t = await getTranslations({ locale, namespace: 'skills-ad-creator' })
  ...
  return (
    <PageShell>
      ...
      <BreadcrumbJsonLd
        items={[
          { name: 'Home', path: '/' },
          { name: 'Ad Creator', path: '/skills/ad-creator' },
        ]}
        locale={locale}
      />
